...
final _float =
    (char('-').optional() & digit().plus() & char('.') & digit().plus())
        .flatten(message: 'floating point expected');

final _exp =
    ((_float | _integer | string('-0')) &
            char('e', ignoreCase: true) &
            anyOf('-+').optional() &
            digit().plus())
        .flatten(message: 'an exponent number expected');

final integer = _integer
    .map(int.parse)
    .where((it) => it >= _intMin && it <= _intMax);
